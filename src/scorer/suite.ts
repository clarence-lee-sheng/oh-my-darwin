...
import { formatExit, parseCommandScore, runScorerCommand } from "./command.js";
...
export async function testSuiteScorer(
  ...
  const result = await runScorerCommand(spec.command, runDir, "test-suite");
  ...
  if (parseRule) {
    ...
    return {
      score: parsed,
      note: parsed === null
        ? `test-suite scorer could not parse a numeric score (${formatExit(result)})`
