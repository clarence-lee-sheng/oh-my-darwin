...
Conducts an **adaptive Socratic interview** (Ouroboros-inspired) that produces `.darwin/meta-spec.md`. Uses the selected engine — `omx --madmax --xhigh` by default, `codex --dangerously-bypass-approvals-and-sandbox` on fallback or explicit Codex request — to dogfood the bridge.
